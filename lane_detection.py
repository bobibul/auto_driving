...
class LKAS:
    def __init__(self):
        ...
        self.car_ctrl_pub = rospy.Publisher("/ctrl_cmd", CtrlCmd, queue_size=3) # 무조건 바꿔야 하는 부분.
...
def get_frame(controller):
    ...
    time.sleep(0.5)
    controller.cam_flag = True
    return color_image

def main():  # 4단계: 메인 함수 정의
    controller = LKAS()  # Class_Name 클래스의 인스턴스를 생성합니다.
    while not rospy.is_shutdown():  # ROS가 종료되지 않는 동안 반복 실행
        color_image = get_frame(controller)
        controller.run(color_image)  # run() 함수를 실행합니다.
